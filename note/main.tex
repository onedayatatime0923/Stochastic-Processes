    ...
    \item Definition
        \begin{itemize}
            ...
            \item Transition Probability Matrix $R$
                ...
                Interpretation:
                \begin{itemize}
                    \item Time in state $i$ before next step is $\sim$ Exponential($v_i$)
                    \item $R_{ij} = \frac{d P[X(t) = j | X(0) = i]}{d t} |_{t = 0}$
                    \item $R_{ij} = \left\{ \begin{array}{cc} -v_i & \text{ if $i == j$} \\ v_i P_{ij} & \text{ if $i == j$} \\ \end{array} \right.$
                \end{itemize}
                Property:
                \begin{itemize}
                    \item $\sum_j R_{ij} = 0$: sum of element is a row of $R$ is 0
                    \item $R$ specifies a unique Continuous Markov Chain
                \end{itemize}
        \end{itemize}
    \item Property
        \begin{itemize}
            \item Poisson process is a special case with $v_i = \lambda, \forall i$
            \item Exploding process: traverse infinite states in finite time, only if $v_i \rightarrow \infty$
            \item Transition from state $i$
                \begin{itemize}
                    \item simulate the transition by multiple exponential variables with $\lambda = v_i \times P_{ij}, \forall j$
                    \item transition to state $j$ if $j$-th random variable hits first
                \end{itemize}
        \end{itemize}
\end{itemize}
